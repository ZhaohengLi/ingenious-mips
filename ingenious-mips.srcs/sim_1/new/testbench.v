...
module TESTBENCH();
	...
	for(i = 0; i < 1025; i = i + 1)begin
	  sopc1.rom1.instractions[i] = 32'h0;
	end
	...
	begin
		...
		$readmemh({ `PATH_PREFIX1, name, ".mem" }, sopc1.rom1.instractions,0,100);
	end
	...
	begin @(negedge CLOCK_50);
		...
		cycle = cycle + 1;
		if(sopc1.cpu1.regWriteAddr_mem_wb_to_reg&& sopc1.cpu1.regWriteEnable_mem_wb_to_reg)
			begin       
